...
const CreateRestaurantForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
	...
	if (newRestaurant.imageFile) {
	  formData.append("imageFile", newRestaurant.imageFile);
	}
  
	try {
	  // Pass formData instead of newRestaurant object
	  await addRestaurant(formData);
	} catch (error) {
	  console.log("Adding restaurant failed:", error);
	}
  };
		...
		>
			...
			<form onSubmit = {handleSubmit} className='space-y-4'>
				...
				<div className='mt-1 flex items-center'>
					...
					{newRestaurant.imageFile && <span className='ml-3 text-sm text-gray-400'>Image uploaded </span>}
				</div>
